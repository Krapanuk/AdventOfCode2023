...
while run:
	...
	posCount = 0
	node = ""
	for pos in Positions: #In the Array at the given position "pos" (maybe convert to int) read the right[2] or left[1] value and get its position
		node = Network[pos][param]
		#print("Step "+str(posCount)+": "+node)
		Positions[posCount] = Indices.index(node)
		if node[2] == "Z": #print(node[2])	#Positions.pop(posCount)
			zCount += 1
		posCount += 1
	if instCount >= len(Instructions)-1: 
		instCount = 0
	else: instCount += 1
	if zCount > 5: run = False 
	#if steps > 5: run = False
	#if steps > 1000000: run = False
	if zCount > 2: print("Positions = "+str(Positions)+" [zCount:"+str(zCount)+"]")
	if zCount > 3: print("Positions = "+str(Positions)+" [zCount:"+str(zCount)+"]")
	if zCount > 4: print("Positions = "+str(Positions)+" [zCount:"+str(zCount)+"]")
	if steps == 10000000: print("Steps > 10000000: "+str(steps)+" [Positions, "+str(Positions)+" zCount:"+str(zCount)+"]")
	if steps == 1000000000: print("Steps > 100000: "+str(steps)+" [Positions, "+str(Positions)+" zCount:"+str(zCount)+"]")
print("RESULT: "+str(steps))

#Result: 27146: Too low
